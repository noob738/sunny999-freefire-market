...
const WEBSITE =
  "https://sunnypaidcolab-collab.github.io/free-fire-account-store/";
...
bot.onText(/^\/start(?:\s+(.+))?$/, async (msg, match) => {
  ...
  await bot.sendMessage(
    ...
    "🎮 Account: " + plan.name + "\n" +
    "💰 Amount: ₹" + plan.amount + "\n\n" +
    "📸 Payment karne ke baad PAYMENT SCREENSHOT yahin send karo.\n\n" +
...
bot.on("photo", async (msg) => {
  ...
  order.status = "PENDING_ADMIN_VERIFICATION";
  order.screenshotFileId = photo.file_id;
  order.screenshotUniqueId = uniqueId;
  ...
  const adminText =
    ...
    "🆔 Order: " + order.id + "\n" +
    "🎮 Account: " + order.planName + "\n" +
    "💰 Amount: ₹" + order.amount + "\n" +
    "👤 User: " + (order.firstName || "Unknown") + "\n" +
...
bot.on("callback_query", async (query) => {
  ...
  if (String(query.from.id) !== ADMIN_CHAT_ID) {
  ...
  if (callback.startsWith("reject_")) {
    action = "reject";
  ...
  if (order.status !== "PENDING_ADMIN_VERIFICATION") {
    await bot.answerCallbackQuery(query.id, {
...
bot.onText(/^\/stock$/, async (msg) => {
  ...
  const pending = data.orders.filter(
    o => o.status === "PENDING_ADMIN_VERIFICATION"
  ).length;

  const approved = data.orders.filter(
    o => o.status === "APPROVED"
  ).length;
...
bot.onText(/^\/orders$/, async (msg) => {
  ...
  const recent = data.orders.slice(-10).reverse();
...
bot.onText(/^\/credentials$/, async (msg) => {
  ...
  const recent = data.credentials.slice(-10).reverse();
...
http.createServer((req, res) => {
  ...
  res.end("SUNNY 999 BOT ONLINE");

}).listen(PORT, () => {
  console.log(
    "SUNNY 999 BOT running on port " + PORT
  );
});

console.log("FFAccountXBot started successfully.");
